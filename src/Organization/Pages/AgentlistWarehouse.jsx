...
export const AgentlistWarehouse = () => {
    ...
    axios
      ...
      .then((res) => {
        if (res.data.status === 1) {
          const formattedData = res.data.warehouse?.map((item) => ({
            ...item,
  ...
  const deleteClick = async (e, row) => {
    try {
      const res = await axios.post(
        `${baseUrl}/Organization/Delete_Agent_By_Organization`,
        {
          agent_id: row.id,
        }
      );
    ...
    <Layout>
      <div>
        <div className="headprofile px-4 pt-3 border-0 text-center mb-4">
          <h3 className="heading3">Warehouse Agent’s List</h3>
