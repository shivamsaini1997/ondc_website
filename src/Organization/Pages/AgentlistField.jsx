    ...
    axios
      ...
      .then((res) => {
        if(res.data.status === 1){
          const formattedData = res.data.filed.map((item) => ({
            ...item,
    ...
    <Layout>
      <div>
        <div className="headprofile px-4 pt-3 border-0 text-center mb-4">
          <h3 className="heading3">Field Agent’s List</h3>
          <span className="sbtitle">Agent Onboarded</span>
          <span className="sonumber">890</span>
        </div>
      </div>
      <div className="text-end px-3">
        <NavLink to={"/organization/both-agent"} className="btnactive">
          Both
        </NavLink>
        <NavLink
          to={"/organization/field-agent"}
