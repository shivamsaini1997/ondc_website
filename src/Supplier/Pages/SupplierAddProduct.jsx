...
function SupplierAddProduct(props) {
    const { id } = useParams();
  ...
  //   const [status, setStatus] = useState(null);
    ...
    useEffect(() => {
      if (categoryid) {
        axios
          .post(`${baseUrl}/Supplier/Get_Product_Sub_category_list_By_Supplier
          `, {
    ...
    const onSubmit = async (data) => {
      ...
      try {
        ...
        formData.append("fk_product_category_id", data.category);
        formData.append("fk_sub_product_id", data.SubCategory);
        formData.append("fk_supplier_id", userId);
        ...
        formData.append("quantity_in_stock", data.Quantity);
      ...
      //   formData.append("status", status);
        ...
        const res = await axios.post(
          `${baseUrl}/Supplier/Add_Product_BY_Individual_Supplier
          `,
          formData
        );
        ...
        if (res.data.status === 1) {
          ...
          navigate(`/supplier/product-list`);
        } else {
                            ...
                            >
                              ...
                              {category?.map((item) => {
                                return (
                                  <option
                                    selected={
                                      data?.category_id === item.id ? true : false
                                    }
                                    value={item.id}
                                  >
                                    {item.category}
                            ...
                            >
                              ...
                              {sabCategory?.map((item) => {
                                return (
                                  <option
                                    selected={
                                      data?.sabCategory_id === item.id ? true : false
                                    }
                                    value={item.id}
                                  >
                                    {item.sub_category}
