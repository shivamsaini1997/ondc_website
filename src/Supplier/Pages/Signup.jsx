...
function SupplierSignup() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            // Signup request
            const signupRes = await axios.post(`${baseUrl}Organization/Organization_Registration`, data);
            ...
            if (signupData.status === 1) {
                ...
                // Login request
                const loginRes = await axios.post(`${baseUrl}Organization/Organization_Login`, {
                    mobile: data.mobile,
                                ...
                                <Form className="form-box" onSubmit={handleSubmit(onSubmit)}>
                                    <Row className="mb-3 admin-fm">
                                        <Form.Group as={Col} md="12 position-relative mb-3" controlId="validationCustom02">
                                            ...
                                            <input className="form-control" defaultValue="" {...register("name", { required: true })} />
                                            {errors.name && <p style={{ color: '#ff0000' }}>Name is required</p>}
                                        </Form.Group>
                                        <Form.Group as={Col} md="12 position-relative mb-3" controlId="validationCustom02">
                                            <Form.Label>Mobile Number</Form.Label>
                                            <input className="form-control" type="mobile" defaultValue="" {...register("mobile", { required: true })} />
                                            {errors.mobile && <p style={{ color: '#ff0000' }}>Mobial is required</p>}
                                        </Form.Group>

                                        <Form.Group as={Col} md="12 position-relative mb-3" controlId="validationCustom03">
                                        ...
                                        <Form.Group as={Col} md="12 position-relative mb-3" controlId="validationCustom04">
                                            ...
                                            <div className="password-input position-relative">
                                                ...
                                                {showConfirmPassword ?
                                                    <FaEyeSlash onClick={() => setShowConfirmPassword(false)} className="eye-icon" />
                                                    :
                                                    <FaEye onClick={() => setShowConfirmPassword(true)} className="eye-icon" />
                                                }
                                            </div>
                                            {errors.confirmPassword && <p style={{ color: '#ff0000' }}>{errors.confirmPassword.message}</p>}
                                        </Form.Group>
                                    ...
                                    <div className="text-end">
                                        <NavLink to={"/supplier/login"} className="color-link">
                                            Already a member?{" "}
                                            <img
